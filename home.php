	<h1 class="media-zen">Media Zen Organic Outreach . Organization</h1>
	<div class="home_container">

		<div id="bp_foundation" class="container-fluid bp_foundation current">
			<div class="row-fluid heading_stripe"><h2>The Process</h2></div>
			<p>The needs of the client are:
				<ul> 
					<li>heard</li> 
					<li>clarified </li>
					<li>developed</li>
					<li>refined</li>
				</ul>
			</p>
			<a name="next"class="btn round pink pull-right" href="#">See how it works >></a>
		</div>
	
		<div id="bp_foundation" class="container-fluid bp_foundation current">
			<div class="row-fluid heading_stripe"><h2>First steps</h2></div>
			<p>The first thing we do is establish <em>what</em> the goals are. 
			We determine project requirements, desires and ultimate vision.</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Then what >></a>
		</div>
		
		<div id="bp_foundation" class="container-fluid bp_foundation current">
			<div class="row-fluid heading_stripe"><h2>Data Deposits</h2></div>
			<p>Identify the entity and cultivate a presence in the datasphere by 
			archiving and sharing useful information in three categories: fun, 
			informative and introspective.
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Read more >></a>
		</div>
	
		<div id="bp_foundation" class="container-fluid bp_foundation current">
			<div class="row-fluid heading_stripe"><h2>Scaleable</h2></div>
			<p>Most projects are at least <em>somewhat</em> budget-constrained, so we develop a scaleable
			strategy, starting by developing the most important elements. The project is structured 
			so that additional features can be added later on.
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">More >></a>
		</div>
	
		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Lasting</h2></div>
			<p>Use frameworks, languages and environments that have stood the test of
			software-time (however brief) so that the foundation will (be more likely to)
			remain relevant in years to come. 
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>
	
		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Reuse</h2></div>
			<p>Usually the functionality we need has already been invented. All we need to do is 
			select the best version to meet our needs and implement it within the Application 
			Framework. 
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>
	
		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Open Source</h2></div>
			<p>Benefit from thousands of hours of international development time
			that continue to develop a host of freely shared popular libraries and frameworks
			that can meet almost any requirement.
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>

		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Lean</h2></div>
			<p>Ever notice a website that takes forever to load the first time? Often half of the 
			code being loaded isn't even necessary to the functionality of the specific site. 
			This can be avoided by compiling <em>only</em> the necessary components into small
			files. 
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>
	
		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Source Control</h2></div>
			<p>Cloud-hosted source/version control records changes to code files
			 over time so that specific versions can be recalled and referenced later. 
			 Development can occur while maintaing a working application.
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>
	
		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Documentation</h2></div>
			<p>Invest now in future development time by writing clear, well documented
			code to take the guess work and sleuthing out of future maintenance, 
			enhancement and refining.
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>

		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Twelve-Factor</h2></div>
			<p>The first thing we do is establish WHAT the goals are. 
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>

		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Green Hosting</h2></div>
			<p>Our servers use 27% less power than the average server because they 
			contain energy efficient components. A portion of hosting proceeds are donated to 
			The Woodland Trust (TWT).
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>

		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Flexible</h2></div>
			<p>The first thing we do is establish WHAT the goals are. 
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>

		<div id="bp_foundation" class="container-fluid bp_foundation" style="display:none">
			<div class="row-fluid heading_stripe"><h2>Sustainable</h2></div>
			<p>The first thing we do is establish WHAT the goals are. 
			</p>
		
			<a name="next"class="btn round pink pull-right" href="#">Next >></a>
		</div>
	</div>




